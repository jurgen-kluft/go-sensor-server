...
func NewUDPServer(connection net.PacketConn, handler MessageHandler, clock Clock, options UDPServerOptions) (*UDPServer, error) {
	...
	server := &UDPServer{
		...
		ctx:        ctx,
		cancel:     cancel,
		queue:      make(chan udpDatagram, options.QueueCapacity),
	}
...
func (server *UDPServer) Serve() error {
	buffer := make([]byte, MessageHeaderSize+MaximumPayloadSize+1)
	for {
		count, remote, err := server.connection.ReadFrom(buffer)
		if err != nil {
		...
		datagram := udpDatagram{data: append([]byte(nil), buffer[:count]...), remote: remote}
		server.queueMu.RLock()
		select {
		case server.queue <- datagram:
...
func (server *UDPServer) worker() {
	...
	for datagram := range server.queue {
		message, err := DecodeDatagram(datagram.data)
		if err != nil {
			server.malformed.Add(1)
			server.options.OnWarning(fmt.Errorf("UDP datagram from %s: %w", datagram.remote, err))
			continue
		}
		...
		if err := server.handler.Route(server.ctx, TransportUDP, timestamp, message); err != nil && !errors.Is(err, ErrUnknownDevice) && !errors.Is(err, ErrUnknownSensor) {
			server.options.OnWarning(fmt.Errorf("route UDP datagram from %s: %w", datagram.remote, err))
		}
	}
}
